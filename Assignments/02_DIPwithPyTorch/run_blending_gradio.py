...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = torch.tensor([[0, 1, 0],
                                     [1, -4, 1],
                                     [0, 1, 0]], dtype=foreground_img.dtype, device=foreground_img.device).unsqueeze(0).unsqueeze(0).expand(1,3,3,3)/6
    
    foreground_img_laplace = torch.nn.functional.conv2d(foreground_img, laplacian_kernel, padding=1)

    blended_img_laplace = torch.nn.functional.conv2d(blended_img, laplacian_kernel, padding=1)

    f = blended_img_laplace.clone()

    f[background_mask==1] =  foreground_img_laplace[foreground_mask==1]
    
    loss = torch.nn.functional.mse_loss(blended_img_laplace,f)
